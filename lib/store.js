...
const DEFAULT_BILLING = {
  plan: "free", // free | artist | studio | founding
  interval: "monthly", // monthly | yearly
  periodStart: null,
  cancelAtPeriodEnd: false,
  cancelEffectiveAt: null, // lapse date promised at cancel time
  pendingPlan: null, // downgrade target, applied when the period ends
  seats: 1,
  onboarded: false,
  frequency: null, // onboarding answer id
  founding: { enabled: true, claimed: 0 },
  passes: [], // [{ id, projectId, purchasedAt, exportsUsed, proposalUsed }]
  usage: { periodKey: null, mockups: 0, proposals: 0, projectsCreated: 0, pdfExports: 0 },
  history: [], // [{ id, at, description, amount }]
};
  ...
  try {
    ...
    state = {
      ...
      billing: {
        ...DEFAULT_BILLING,
        ...parsed.billing,
        founding: { ...DEFAULT_BILLING.founding, ...parsed.billing?.founding },
        usage: { ...DEFAULT_BILLING.usage, ...parsed.billing?.usage },
      },
    };
  } catch {
...
export const actions = {
  ...
  billingPatch(patch) {
    const s = load();
    commit({ ...s, billing: { ...s.billing, ...patch } });
  },

  billingSubscribe(planId, interval, amount, description) {
    const s = load();
    const founding = { ...s.billing.founding };
    if (planId === "founding") founding.claimed = (founding.claimed || 0) + 1;
    const next = {
      ...s,
      billing: {
        ...s.billing,
        plan: planId,
        interval: planId === "founding" ? "yearly" : interval,
        ...
        founding,
        history: [
          { id: newId(), at: new Date().toISOString(), description, amount },
          ...s.billing.history,
        ],
      },
    };
    logActivity(next, "billing", `Subscribed — ${description}`);
    commit(next);
  },

  billingCancel(effectiveAt) {
    const s = load();
    commit({
      ...s,
      billing: { ...s.billing, cancelAtPeriodEnd: true, cancelEffectiveAt: effectiveAt || null },
    });
  },
  ...
  billingResume() {
    ...
    commit({
      ...s,
      billing: { ...s.billing, cancelAtPeriodEnd: false, cancelEffectiveAt: null, pendingPlan: null },
    });
  },

  billingApplyPeriodEnd(planId) {
    // Called on read when a canceled/downgraded period has lapsed.
    const s = load();
    commit({
      ...s,
      billing: {
        ...
        cancelEffectiveAt: null,
        pendingPlan: null,
        periodStart: planId === "free" ? null : new Date().toISOString(),
      },
    });
  },

  billingPurchasePass(projectId, amount, description) {
    const s = load();
    const pass = {
      id: newId(),
      projectId,
    ...
    logActivity(next, "billing", `Project Pass unlocked`);
    commit(next);
    return pass;
  },

  billingUpdatePass(passId, patch) {
    const s = load();
    commit({
      ...s,
      billing: {
        ...s.billing,
        passes: s.billing.passes.map((p) => (p.id === passId ? { ...p, ...patch } : p)),
      },
    });
  },

  billingRecordUsage(kind) {
    const s = load();
    const key = usagePeriodKey(s.billing);
    const usage =
      s.billing.usage.periodKey === key
        ? { ...s.billing.usage }
        : { periodKey: key, mockups: 0, proposals: 0, projectsCreated: 0, pdfExports: 0 };
    usage[kind] = (usage[kind] || 0) + 1;
    commit({ ...s, billing: { ...s.billing, usage } });
  },
};

/**
 * Monthly-allowance cycle key. Subscribers reset on their billing anchor day;
 * free accounts reset with the calendar month.
 */
export function usagePeriodKey(billing) {
  if (billing?.periodStart) {
    ...
    return `sub-${billing.periodStart}-${Math.max(0, months)}`;
  }
  const now = new Date();
  return `cal-${now.getFullYear()}-${now.getMonth() + 1}`;
}
