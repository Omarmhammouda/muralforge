...
export function parseInviteCodes() {
  const raw = readEnv("INVITE_CODES") || "";
  const codes = new Map();
  for (const entry of raw.split(",")) {
    const [code, allowance] = entry.split(":").map((part) => (part || "").trim());
    if (!code) continue;
    if (/^unlimited$/i.test(allowance || "")) {
      codes.set(code, Infinity);
      continue;
    }
    const parsed = Number.parseInt(allowance ?? "", 10);
    if (Number.isFinite(parsed) && parsed > 0) codes.set(code, parsed);
  }
  return codes;
}
...
export function consumeGeneration(state, inviteCode) {
  ...
  if (inviteCode && codes.has(inviteCode)) {
    ...
    if (allowance === Infinity) {
      // Owner/unlimited code: nothing to count, nothing to store.
      return { allowed: true, next: state, remaining: null, tier: "code" };
    }
